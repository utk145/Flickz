...
export default function MovieList({ title, data, hidden }) {
  ...
  // const movieName = "Movie is choosen as this";
  return (
    <View className="mb-8 space-y-4">
      <View className="mx-4 flex-row items-center justify-between">
        <Text className="text-white text-xl font-semibold">{title}</Text>
        <TouchableOpacity>
          <Text style={styles.text} className="text-xl">{hidden ? "" : "See All"}</Text>
        </TouchableOpacity>
      </View>
          ...
          data.map((item, index) => (
            <TouchableWithoutFeedback key={index} onPress={() => nav.push("Movie", item)}>
              <View className="space-y-1 mr-4">
                {/* <Image source={require("../assets/images/moviePoster1.png")} className="rounded-3xl" style={{ width: width * 0.33, height: height * 0.22 }} />
